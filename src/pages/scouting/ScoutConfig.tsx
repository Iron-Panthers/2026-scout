...
export default function ScoutConfig() {
    ...
    useEffect(() => {
        ...
        const loadWithoutId = async () => {
          ...
          //   if (teamNumber) {
          //     teamNumbersMap[`${match_id}-${match_role}`] = teamNumber;
          //     console.log(teamNumber + " " + `${match_id}-${match_role}`)
          //   }
          // }
    ...
    return (
      <div className="space-y-4 py-4" style={ { padding: '20px' } }>
        <Button onClick={() => navigate(`/scout/lobby/${selectedMatch.match.id}`)} size="lg"
            className="h-12 text-lg font-semibold flex flex-col gap-2" style={ { width: '100%' } }>
          Start Scouting
        </Button>
        <div className="flex w-full max-w-sm flex-col gap-6">
          <Tabs defaultValue="config">
            <TabsContent value="config">
              {/* Robot Image */}
              <div className="w-full h-48 bg-muted rounded-lg flex items-center justify-center border-2 border-border overflow-hidden mb-3">
                {loadingPhoto ? (
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center pl-3 p-1 bg-accent/50 rounded-lg">
                  ...
                  <Input className="font-semibold" className='w-30 text-right' type='number' placeholder='#####' onInput={(e) => { setMatchNumber(e.currentTarget.value) }} value={matchNumber}/>
                </div>
                <div className="flex justify-between items-center pl-3 p-1 bg-accent/50 rounded-lg">
                  <span className="text-sm font-medium text-muted-foreground">
                    Team Number
                  </span>
                  <Input className="font-semibold" className='w-30 text-right' type='number' placeholder='#####' onInput={(e) => { setTeamNumber(e.currentTarget.value) }} value={teamNumber} />
                </div>
